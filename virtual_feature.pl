...
# rename return line to https instead.
my @sreturn = &virtualmin_nginx::find("return", $server);
my $domm = $d->{'dom'};
	foreach my $l (@sreturn) {
    	if ($l->{'words'}->[1] =~ /\Q$domm\E/) {
        	&virtualmin_nginx::save_directive($server, [ $l ], [ ]);
            my $returna = { 'name' => 'return',
		        	'words' => [ '301', 'https://www.'.$d->{'dom'}.'$request_uri' ]
               	  };
			&virtualmin_nginx::save_directive($server, [ ], [ $returna ]);
        }
    }
